...
For Project mutations, the service flushes and refreshes the entity while
the transaction is active, constructs the detached `ProjectResponse`, and
commits as the final database operation. Nothing reads from the database
after commit, so a refresh failure cannot follow a durable write and a locked
mutation returns the state it captured before releasing its row lock.
...
- Acquiring row locks for mutation workflows that must serialize lifecycle checks
...
FastAPI generates an OpenAPI schema from routes and Pydantic models. That schema flows into the frontend as generated contract types and generated operation functions, which delegate to the existing handwritten transport:

backend/openapi.json
→ generated schema types and operation functions
→ existing client/server transport
→ frontend features
...
`frontend/lib/api/generated/` owns the TypeScript representation of that contract, produced entirely by `pnpm api:generate` and committed to the repository; neither of its two files is ever hand-edited:

- `schema.ts` — response and schema types, generated by [`openapi-typescript`](https://openapi-ts.dev/) with server-defaulted properties kept optional, which emits types only (no runtime, no fetch client).
- `operations.ts` — one typed operation function per OpenAPI operation (e.g. `healthGet`), generated by `frontend/scripts/generate-api-operations.mjs`. Endpoint paths and HTTP methods are generated as explicit, tested metadata alongside each function, so path or method drift is caught the same way response-shape drift already was. Required JSON request bodies become required schema-derived function arguments and are forwarded to the transport; bodyless operation signatures remain unchanged. Request execution itself remains entirely handwritten: every generated function takes an injected transport argument compatible with the existing `apiRequest`, and delegates to it rather than calling `fetch()` itself. No generated fetch runtime or second transport exists anywhere in `frontend/lib/api/generated/`.

`frontend/lib/api/contracts.ts` re-exports only the currently-used types and operation functions (e.g. `HealthResponse`, `healthGet`) as a narrow, stable boundary; feature code imports from there, never from the generated files directly, and calls a generated operation function with the real `apiRequest` — for example `healthGet(apiRequest, { requestId })` — rather than writing the endpoint path by hand. This is what "generate the frontend API client" means in this repository: generated contract types and operation metadata, executed by the unchanged handwritten client/server/error-normalization layers, never a replacement for them.

When a backend request or response changes, `backend/openapi.json` and the generated frontend client must both be regenerated. `pnpm api:check` verifies this deterministically: it runs `pnpm openapi:check`, then regenerates `schema.ts` and `operations.ts` into a temporary directory — using the exact same `openapi-typescript` binary and `generate-api-operations.mjs` logic `pnpm api:generate` uses — and compares each byte-for-byte against the committed copy in `frontend/lib/api/generated/`. It fails, naming the specific stale, missing, or unexpectedly extra file, whenever the committed contract and the committed generated client have drifted apart. Generation and freshness verification are deliberately separate commands with a strict one-way relationship: `pnpm api:generate` is the only command that writes to `frontend/lib/api/generated/`; `pnpm api:check` only ever reads it and never regenerates or overwrites it, even on failure.
