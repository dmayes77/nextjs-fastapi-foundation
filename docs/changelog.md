...
- Added a generated, committed frontend API contract: `pnpm api:generate` runs `openapi-typescript` against `backend/openapi.json` to produce type-only TypeScript (`frontend/lib/api/generated/schema.ts`, never hand-edited, no runtime of its own) without requiring FastAPI to be running; `frontend/lib/api/contracts.ts` re-exports stable named types such as `HealthResponse` that the existing health integration now uses, so the generated contract supplements the handwritten `lib/api/client.ts`/`server.ts` transport and `normalizeError()` rather than replacing them.
