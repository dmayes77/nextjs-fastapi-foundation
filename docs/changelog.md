...
- Added repository-wide and path-specific GitHub instructions for consistent Copilot, Codex, backend, frontend, and documentation behavior.
- Added a repository-wide `AGENTS.md` for AI coding agents, deferring to more specific nested files such as `frontend/AGENTS.md`, and clarified `docs/architecture.md` to distinguish it from the separate GitHub Copilot custom instructions under `.github/`.
