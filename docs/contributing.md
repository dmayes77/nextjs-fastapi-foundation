...
The project is a reusable GitHub template, so contributions must preserve clarity, simplicity, and predictable setup.
...
Use the structured bug and feature forms in the repository's Issues tab. Blank
issues are disabled so reports include the context maintainers need. Security
vulnerabilities must be reported privately through the process in
[SECURITY.md](../SECURITY.md), never through a public issue.

Pull requests automatically receive the repository template. Complete the
impact sections and only check conditional validation items when they apply.
Changes to `main` must go through a pull request, pass the required
`Quality, PostgreSQL, and Playwright` check, and have review conversations
resolved.
...
Do not edit an applied shared migration to make a later schema change.
